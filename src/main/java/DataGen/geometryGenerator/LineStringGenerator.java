...
public class LineStringGenerator extends GeometryGenerator {
	...
	/**
	 * Create the random points in a bounding box.
	 *
	 * Note: this requires the number of points to be greater than 2.
	 *
	 * @see #getNumberPoints()
	...
	public Geometry create() {
		...
		for(int i=0;i<RUNS;i++){
			...
			case ARC:
				...
				break;
			case RND:
				fillRnd(x,dx,y,dy,coords,geometryFactory);
				break;
			default:
	...
	private static void fillRnd(double x, double dx, double y, double dy, Coordinate[] coords, GeometryFactory gf){
		if(coords.length == 2)
			throw new IllegalStateException("Too few points for Rdm");

		for(int i=0;i<coords.length;i++){
			double fx = HelperClass.getRandomDoubleInRange(x, x + dx);
			double fy = HelperClass.getRandomDoubleInRange(y, y + dy);

			coords[i] = new Coordinate(fx,fy);
			gf.getPrecisionModel().makePrecise(coords[i]);
		}
	}
