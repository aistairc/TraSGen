	...
	public Geometry create() {
		...
		for(int i=0;i<RUNS;i++){
			...
			case ARC:
				...
				break;
			case RND:
				p = createRnd(x,dx,y,dy,numberHoles,numberPoints,geometryFactory);
				break;
			default:
...
//	static double notValid = 0;
	private static Polygon createRnd(double x, double dx, double y, double dy, int nholes, int npoints, GeometryFactory gf){
		// make outer ring first
		LinearRing outer;
		while (true) {
			outer = createRnd(x, dx, y, dy, npoints, gf);
//			count += 1;
			if (gf.createPolygon(outer, null).isValid()) {
				break;
			}
//			notValid += 1;
		}
//		System.out.println("count = " + count + ", notValid = " + notValid + ", notValid/count = " + (notValid/count)*100);
		...
		// make inner ring
		ArrayList<LinearRing> innerList = new ArrayList<>();
		for (int i = 0; i < npoints; i++) {
			LinearRing ring = createInnerRnd(x, dx, y, dy, outer, innerList, gf);
			if (ring != null) {
				innerList.add(ring);
			}
		}
		LinearRing[] inner = innerList.toArray(new LinearRing[innerList.size()]);
		return gf.createPolygon(outer, inner);
	}

	private static LinearRing createRnd(double x, double dx, double y, double dy, int npoints, GeometryFactory gf){

		Coordinate[] coords = new Coordinate[npoints+1];
		Coordinate[] randomCoords = new Coordinate[npoints];

		for(int i = 0; i < npoints; i++) {
			double fx = HelperClass.getRandomDoubleInRange(x, x + dx);
			double fy = HelperClass.getRandomDoubleInRange(y, y + dy);

			randomCoords[i] = new Coordinate(fx,fy);
			gf.getPrecisionModel().makePrecise(randomCoords[i]);
		}

		Coordinate[] sortedCoords = HelperClass.sortPolygonCoordinateByAngle(randomCoords);

		for(int i = 0; i < sortedCoords.length; i++) {
			coords[i] = sortedCoords[i];
		}

		coords[npoints] = new Coordinate(sortedCoords[0]);
		gf.getPrecisionModel().makePrecise(coords[npoints]);

		return gf.createLinearRing(coords);
	}

	private static LinearRing createInnerRnd(double x, double dx, double y, double dy,
											 LinearRing outer, ArrayList<LinearRing> inner, GeometryFactory gf){
		final int POINTS = 3;
		...
		while (true) {
			for (int i = 0; i < POINTS; i++) {
				double fx = HelperClass.getRandomDoubleInRange(x, x + dx);
				double fy = HelperClass.getRandomDoubleInRange(y, y + dy);
				Coordinate c = new Coordinate(fx, fy);
				// whether it is inside the outer
				if (!SimplePointInAreaLocator.containsPointInPolygon(c, gf.createPolygon(outer, null))) {
					// outside
					i--;
					continue;
				}
			...
			ArrayList<LinearRing> list = (ArrayList<LinearRing>)inner.clone();
			list.add(gf.createLinearRing(coords));
			LinearRing[] rings = list.toArray(new LinearRing[list.size()]);
			IsValidOp valid = new IsValidOp(gf.createPolygon(outer, rings));
			if (!valid.isValid()) {
				if (retry++ > MAX_RETRY) {
					return null;
				}
